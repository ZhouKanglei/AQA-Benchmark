...
class Graph(object):
    ...
    def get_binary_adj(self):
        ...
        for i in range(self.num_node):
            for j in range(self.num_node):
                if (i - j < self.num_k and i - j >= 0) or (
                    j - i < self.num_k and j - i >= 0
                ):
                    self.adj[i, j] = 1
                else:
                    self.adj[i, j] = 0
    ...
    def normalize_adj(self):
        ...
        self.norm_adj = (norm_degs_matrix @ self.adj @ norm_degs_matrix).astype(
            np.float32
        )
...
class attAdj(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, num_nodes, num_groups=4, softmax=True
    ):
        ...
        self.conv_q = nn.Conv1d(
            in_channels, out_channels, kernel_size=(1,), groups=num_groups, bias=False
        )
...
class clipRefine(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, num_groups=1, num_clips=10, num_k=3, factor=2
    ):
...
class shotGCN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, A, num_groups=4, factor=2):
        ...
        self.A = A
        self.adp_adj = attAdj(
            in_channels, mid_channels, A.shape[0], num_groups=num_groups, softmax=False
        )
...
class diffGCN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, A, num_groups=1, factor=1):
        ...
        self.S = nn.Parameter(
            torch.ones((out_channels, A.shape[0])), requires_grad=True
        )
        self.alpha = nn.Parameter(torch.eye(1), requires_grad=True)
...
class motionGCN(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, num_groups=4, num_scenes=10, factor=4
    ):
        ...
        self.adp_A = attAdj(
            in_channels, mid_channels, num_scenes, num_groups=num_groups, softmax=False
        )
        self.softmax = nn.Softmax(-1)
        # aggregation
...
class HGCN(nn.Module):
    ...
    def __init__(
        ...
        # Clip refinement
        self.shot_gen = (
            clipRefine(in_channels, 512, num_groups, num_clips, num_k, factor)
            if a
        ...
        # Motion graph aggregation
        self.mot_agg = (
            motionGCN(256, 128, num_groups, num_scenes, factor)
            if c
